...
export const getCustomers = async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE u.role IN ('customer','collaborator')
...
/* ---------------------------------------------------
   Create Collaborator (Customer Company Member)
--------------------------------------------------- */
export const createCollaborator = async (req, res) => {
  ...
  try {
    ...
    // 1️⃣ Email must be globally unique
    ...
    Promise.resolve().then(async () => {
      try {
        await sendCustomerCredentials({
          ...
          name,
          tempPassword,
          accountType: "Collaborator Account",
        });
      } catch (err) {
        console.error("Collaborator email failed:", err);
      }
    });
    ...
    await client.query("ROLLBACK");
    console.error("CreateCollaborator Error:", err);
    res.status(500).json({ message: "Server error" });
  } finally {
    client.release();
  }
...
/* ---------------------------------------------------
   Delete collaborator (Customer Company Member)
--------------------------------------------------- */
export const deleteCollaborator = async (req, res) => {
  ...
  try {
    ...
    // 1️⃣ detach documents from this user
    await client.query(
    ...
    res.json({ message: "Collaborator deleted permanently" });
  } catch (err) {
    await client.query("ROLLBACK");
    console.error(err);
    res.status(500).json({ message: "Server error" });

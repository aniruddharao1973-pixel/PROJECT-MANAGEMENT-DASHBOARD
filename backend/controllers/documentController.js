...
export const uploadDocument = async (req, res) => {
  ...
  if (req.user.role === "customer" || req.user.role === "collaborator") {
  ...
  for (const file of uploadedFiles) {
    ...
    ------------------------------------------------------------------ */
      ...
      // const normalizedPath = file.path.replace(/\\/g, "/"); // Windows safe (vedio not played)
      // const filePath = `/${normalizedPath}`;

      // file.path example:
      // F:\PM DASHBOARD MICROLOGIC\...\backend\uploads\11-02-2026\video.mp4
      ...
      // Now relativePath = 11-02-2026/video.mp4

      const normalizedPath = relativePath.replace(/\\/g, "/");

      const filePath = `/uploads/${normalizedPath}`;

      // Final result:
      // /uploads/11-02-2026/video.mp4
...
============================================================ */
      ...
      AND u.role IN ('customer','collaborator')
...
NULL::text AS signature_type,
    ...
    if (["customer", "collaborator"].includes(req.user?.role)) {
...
------------------------------------------------------------ */
  ...
  if (
    ["customer", "collaborator"].includes(req.user.role) &&
    access.allow_customer_sign !== true
  ) {
    return res.status(403).json({

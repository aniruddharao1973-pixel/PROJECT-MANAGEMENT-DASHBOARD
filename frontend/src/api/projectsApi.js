...
export const useProjectsApi = () => {
   ...
   */
  const getProjectsByCompany = useCallback(
    (companyId) => {
  ...
  /**
   * Department-specific:
   * - fetch projects assigned to the logged-in department
   * - department is resolved from JWT on backend
   */
  const getDepartmentProjects = useCallback(
    (departmentId) => {
      // ⛔ Guard against null / undefined / empty
      if (!departmentId) {
        console.warn("⚠️ getDepartmentProjects called without departmentId");
        return Promise.resolve({ data: [] });
      }

      return api.get(`/projects/department/${departmentId}/projects`);
    },
    [api],
  );
  ...
  const renameProject = (projectId, name) =>
    api.put(`/projects/${projectId}/rename`, { name });
  return {
    createProject,
    getAllProjects,
    getProjectById,

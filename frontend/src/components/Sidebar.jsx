...
const Sidebar = ({ sidebarContext, sidebarOpen, onClose }) => {
  ...
  if (user?.role === "department") {
    menu = departmentMenu;
  } else if (sidebarContext === "teams" && isAdminLike) {
    menu = teamsMenu;
  } else if (user?.role === "customer" || user?.role === "collaborator") {
    menu = customerMenu;
  } else {
    menu = adminMenu;
  }
    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-gradient-to-b from-[#3b82f6] via-[#4f46e5] to-[#4338ca]
        ...
        style={{
          boxShadow:
            "0 25px 50px -12px rgba(99, 102, 241, 0.4), 0 0 80px rgba(79, 70, 229, 0.35)",
        }}
        aria-label="Sidebar"
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-0 left-0 w-full h-full opacity-30">
            ...
            <div
              className="absolute top-1/3 right-5 w-16 h-16 bg-indigo-300/10 rounded-full blur-xl animate-pulse"
        ...
        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-indigo-600/50 to-transparent pointer-events-none"></div>
      </aside>

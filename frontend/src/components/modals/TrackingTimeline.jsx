...
const TrackingTimeline = ({ document, timeline = [], version = null }) => {
  ...
  const isCustomer = ["customer", "collaborator"].includes(safeRole);
    ...
    <div className="relative w-full min-h-0">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white to-slate-50/50 shadow-sm p-4 sm:p-5">
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <ProgressConnector
                active={
                  progress?.hasUploaded &&
                  (isDirectPublish || progress?.hasSubmitted)
                }
              />
              ...
              {!isDirectPublish && (
                <>
                  ...
                  <ProgressStep
                    label={progress?.hasRejected ? "Rejected" : "Approved"}
                    sublabel={
                      progress?.hasRejected ? "Needs changes" : "Verified"
                    }
                    active={progress?.hasApproved || progress?.hasRejected}
                    completed={progress?.hasApproved}
                    danger={progress?.hasRejected}
      ...
      <div className="relative pl-6 sm:pl-8">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, rgba(139, 92, 246, 0.3) 0%, rgba(99, 102, 241, 0.15) 50%, rgba(139, 92, 246, 0.05) 100%)",
          }}
        />
                  ...
                  {isInternal && (
                    <details className="group/details border-b border-slate-100/60">
                      <summary className="flex items-center gap-2 cursor-pointer px-3.5 sm:px-4 py-2.5 text-xs text-slate-400 hover:text-slate-600 hover:bg-slate-50/50 transition-colors duration-200">
                        <svg
                          className="w-3.5 h-3.5 transition-transform duration-200 group-open/details:rotate-90"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                      ...
                      <div className="px-3.5 sm:px-4 py-3 bg-slate-50/50">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
                          ...
                          {item.approval_action && (
                            <MetaItem
                              label="Action"
                              value={item.approval_action}
                            />
                          )}
                          <div className="col-span-2">
                            <MetaItem
                              label="Timestamp"
                              value={new Date(item.event_time).toISOString()}
                            />
                          </div>
                        </div>
                      </div>
...
const ProgressStep = ({
  ...
  <div
    className={`flex flex-col items-center gap-2 ${first || last ? "flex-shrink-0" : "flex-1"}`}
  >
    <div
      className={`
